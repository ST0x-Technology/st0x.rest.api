...
//!
//! We replay a real order from st0x's orderbook (captured from the production
//! Goldsky subgraph, `tests/fixtures/alb_usdc_order.json`) that sells `ALB-WR1-R1`
//! (`0xf836a5…`) for `USDC` (`0x833589…`) on orderbook `0xe522cB4a…` — the
//! orderbook and subgraph st0x's `base` registry resolves to. The order's real
//! `orderBytes` are served to the calldata builder via a mock subgraph HTTP
...
//! oracle) over oracle-priced `wt*` orders on the same orderbook: an oracle order
//! reverts with `StalePrice` when quoted on a pinned fork (the oracle's freshness
//! window is shorter than the fork's block age), which is a property of the
//! oracle, not the swap calldata we are trying to prove.
//!
//! ## Env gating
//!
...
/// st0x orderbook (Raindex) on Base — the address st0x's `base` registry uses.
