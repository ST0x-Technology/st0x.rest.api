...
1. **List tokens** — `GET /v2/tokens?chainId=<chain-id>` to discover available
   trading pairs
2. **Get a quote** — `POST /v3/swap/quote` with `chainId` to see estimated
   pricing
3. **Get calldata** — `POST /v3/swap/calldata` to generate the transaction
...
You can also inspect existing orders and monitor their trades:

1. **Monitor** — `GET /v2/order/{order_hash}?chainId=<chain-id>` for status,
   `GET /v2/trades/{address}?chainId=<chain-id>` for fills
2. **Cancel** — `POST /v2/order/cancel` with `chainId` to get cancellation
   calldata, then execute on-chain

Order creation is not currently exposed through the V2 API.
