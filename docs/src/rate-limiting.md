...
The configured per-key limit is the default for every API key. Operators can
assign a positive requests-per-minute override to an individual key with the
`keys set-rate-limit` CLI command, or remove it with `keys clear-rate-limit`.
The global limit continues to apply when a key has a higher override.
